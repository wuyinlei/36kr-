...
public class ArticleDataManager {
	 ...
	 * @return
	 */
	public ArticleBean getArticleBean_New(Document document){
		ArticleBean articleBean=new ArticleBean();
		//首先获取局部的文章相关数据
		Element singleElement=document.select("div.js-react-on-rails-component").first();
		String data_props=singleElement.attr("data-props");
		//开始抓取用户信息
		String author_Str=HttpRequest.sendPost("http://36kr.com/asynces/posts/author_info", "url_code="+articleId);
		AuthorBean bean=null;
		try {
			JSONObject data=new JSONObject(data_props).getJSONObject("data");
			JSONObject post=data.getJSONObject("post");
			articleBean.setTitle(post.getString("title"));
			articleBean.setContext(post.getString("display_content"));
			articleBean.setDatetext(post.getString("published_at"));
			articleBean.setDatetime(post.getString("published_at"));
			articleBean.setHeadImage(post.getString("cover"));
			articleBean.setTagBeans(null);
    ...
    public ArticleBean getArticleBean(Document document){
    	ArticleBean articleBean=new ArticleBean();
    	//首先获取局部的文章相关数据
    	Element singleElement=document.select("article-section").first();
    	//获取标题
    	//single-post__title
